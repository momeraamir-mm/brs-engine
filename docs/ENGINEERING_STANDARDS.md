...
- **Tasks / threading:** the browser/Electron build runs the engine in a worker (SceneGraph **Tasks work**). The headless Node CLI **also runs the engine in a worker thread** (see `src/cli/app-worker.ts`; decision D43) precisely so SceneGraph **Tasks run headless too** — the old "main-thread, Tasks don't run" model starved the event loop and was replaced. Verified 2026-06-17: AudioShelf's `NetworkTask` loads `pkg:/data/catalog.json` under `test-loop.py` → telemetry `catalogState=done, items=3`. (Historical note: earlier revisions of this doc said Tasks don't run headless — stale since D43.)
